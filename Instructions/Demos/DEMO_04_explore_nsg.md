...
1. 그러면 VM 구성의 네트워킹 탭이 표시됩니다.  NIC 네트워크 보안 그룹 옵션을 **없음**으로 선택합니다. 다른 모든 설정은 기본값을 유지합니다.
...
1. **개요** 탭으로 다시 이동합니다.  페이지 오른쪽 위에 "연결 대상: 서브넷 0개, 네트워크 인터페이스 1개"가 표시됩니다. 즉, NSG에 인터페이스가 할당된 것입니다. 기본 규칙에 대해서도 부연 설명합니다. 인바운드 트래픽의 경우 다른 Azure Virtual Network 및 Azure Load Balancer에서 전송된 트래픽만 허용됩니다. VM으로 전송되는 기타 모든 인바운드 트래픽은 거부됩니다. 기본 아웃바운드 규칙에 대해서도 설명합니다.  다른 VNet으로 전송하는 아웃바운드 트래픽 및 아웃바운드 인터넷 트래픽만 허용됩니다.  데모 중에 잠시 시간을 할애하여 인바운드 트래픽이 거부되는 방식을 시연하는 것이 좋습니다.
    ...
    1. 포트 필수 조건이 충족되지 않습니다.  필수 조건을 충족하려면 RDP에서 사용하는 대상 포트 3389가 있는 인바운드 네트워크 보안 규칙을 구성해야 합니다.  

1. 이제 인바운드 RDP 트래픽을 허용하는 새 규칙을 만듭니다.  기존 기본 규칙을 삭제할 수는 없으며 우선 순위가 더 높은 새 규칙을 만들어야 함을 설명합니다. 왼쪽 탐색 패널의 설정 아래에서 **네트워킹**을 선택합니다.  현재 VM의 네트워킹 페이지에 있습니다.
1. **인바운드 포트 규칙** 탭이 선택되었는지(밑줄이 그어져 있는지) 확인하고, **인바운드 포트 규칙 추가**를 선택하여 다음 설정을 이용해 인바운드 RDP 트래픽을 허용하는 규칙을 만듭니다.
    1. 원본: **모두**
    1. 원본 포트 범위: **\***
    1. 대상: **모두**
    1. 서비스: **RDP**
...
1. 이제 **아웃바운드 포트 규칙** 탭을 선택하고 기본 규칙을 검토합니다.  페이지 위쪽에서 **아웃바운드 포트 규칙 추가**를 선택하고 다양한 설정에 대해 설명합니다.  마찬가지로 규칙을 직접 만들어 보는 것이 좋습니다. 아래 설정을 사용하는 경우 아웃바운드 인터넷 트래픽을 거부하는 규칙이 작성됩니다.
    1. 원본: **모두**
    1. 원본 포트 범위: **\***
    1. 대상: **서비스 태그**
    1. 대상 서비스 태그: **인터넷**
...
1. 이제 VM으로 돌아와 규칙을 테스트합니다.  페이지 위쪽에서 **SC900-VM**을 선택합니다.
