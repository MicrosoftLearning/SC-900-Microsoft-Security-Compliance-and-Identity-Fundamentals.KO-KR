<!---
---
랩: 제목: 'Azure NSG(네트워크 보안 그룹) 살펴보기' 학습 경로/모듈/단원: '학습 경로: Microsoft 보안 솔루션의 기능 설명, 모듈 1: Azure의 기본 보안 기능 설명, 단원 6: Azure 네트워크 보안 그룹 설명'
---
--->
...
1. 왼쪽 탐색 패널에서 네트워크 설정** 선택합니다**.  기본 창의 필수 섹션에는 VM에 대한 네트워크 인터페이스가 표시됩니다.  인터페이스에 할당된 NSG가 없으므로 네트워크 보안 그룹 옆에 나열된 항목이 없는 방법을 확인합니다.
...
1. 열린 Azure 탭*에서 페이지 맨 위에 있는 홈** 링크를 마우스 오른쪽 단추로 클릭하고* **새 탭**에서 링크 열기를 선택하여 **Azure 서비스에 대한 다른 페이지를 엽니다.

1. 페이지 위쪽의 파란색 검색 창에 **네트워크 보안 그룹**을 입력하고 결과에서 **네트워크 보안 그룹**을 선택합니다. *네트워크 보안 그룹(클래식)* 을 선택하면 안 됩니다.

1. 페이지 가운데에서 네트워크 보안 그룹** 만들기 레이블이 지정된 **파란색 단추를 선택합니다.  또는 네트워크 보안 그룹 페이지의 맨 위에서 + 만들기**를 선택할 **수 있습니다.
...
1. 새로 만든 NSG의 개요 페이지에 있어야 합니다.  그렇지 않은 경우 왼쪽 탐색 패널에서 개요**를 선택합니다**. 페이지 위쪽의 필수 아래에서 방금 만든 NSG 관련 기본 정보를 확인할 수 있습니다.  두 가지 유의할 점은 사용자 지정 보안 규칙이 없으며 이 NSG와 연결된 서브넷이나 네트워크 인터페이스가 없다는 것입니다.  사용자 지정 보안 규칙은 없지만 페이지에 표시된 대로 모든 NSG에 포함된 기본 인바운드 및 아웃바운드 규칙이 있습니다.  인바운드 및 아웃바운드 규칙을 모두 검토합니다. 기본 인바운드 규칙은 가상 네트워크 또는 Azure 부하 분산 장치에서 전송되지 않은 모든 인바운드 트래픽을 거부합니다.  아웃바운드 규칙은 가상 네트워크 간 트래픽과 인터넷으로의 아웃바운드 트래픽을 제외한 모든 아웃바운드 트래픽을 거부합니다.

1. NSG-SC900 페이지 왼쪽 탐색 창의 설정 아래에서 **네트워크 인터페이스**를 선택합니다.
    1. **연결**을 선택합니다.
    2. 네트워크 인터페이스 연결 필드에서 **아래쪽 화살표**를 선택하고 **sc900-winvmXXX**를 선택한 다음 창 아래쪽에서 **확인**을 선택합니다. NSG에 연결된 인터페이스는 목록에 표시됩니다.  이제 NSG가 VM의 네트워크 인터페이스에 할당됩니다.

1. 브라우저에서 **SC900-WinWM - Microsoft Azure** 탭으로 다시 전환합니다.  페이지를 새로 고칩니다. 네트워크 보안 그룹이 표시된 위치 옆에 이제 방금 만든 NSG의 이름이 표시됩니다.  그래도 표시되지 않으면 잠시 기다린 다음 페이지를 다시 새로 고칩니다.

1. 왼쪽 탐색 패널에서 커넥트** 선택합니다**. 기본 창의 포트 번호 3389가 표시된 위치 옆에 있는 액세스 확인(Check access **)을 선택합니다**. 검사 액세스 함수는 액세스 가능한 경우 검사 위해 VM의 기본 RDP 포트 3389로 신호(트래픽)를 보냅니다. 1분 정도 걸릴 수 있지만 액세스할 수 없습니다.  DenyAllInBound NSG 규칙은 VM에 대한 모든 인바운드 트래픽을 거부하기 때문에 이 작업이 필요합니다.
