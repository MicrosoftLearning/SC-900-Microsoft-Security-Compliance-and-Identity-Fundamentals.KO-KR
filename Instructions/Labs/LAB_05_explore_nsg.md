...
1. NSG-SC900 페이지 왼쪽 탐색 창의 설정 아래에서 **네트워크 인터페이스**를 선택합니다.
    1. **연결**을 선택합니다.
    2. 네트워크 인터페이스 연결 필드에서 **아래쪽 화살표**를 선택하고 **sc900-winvmXXX**를 선택한 다음, 창 아래쪽에서 **확인**을 선택합니다. NSG에 연결된 인터페이스는 목록에 표시됩니다.
...
1. 페이지 맨 위에서 **추가**를 선택합니다. 인바운드 보안 규칙 추가 창에서 다음 설정을 지정합니다.
    ...
    1. 서비스:  **RDP**
    1. 작업:  **허용**
    1. 우선 순위:  **1000**. 참고: 번호가 낮은 규칙이 우선 순위가 더 높으며 먼저 처리됩니다.
    1. 이름: 기본 이름을 그대로 두거나 고유한 설명이 포함된 이름을 만듭니다.
    1. 페이지 아래쪽에 경고 기호를 확인합니다.  NSG의 기능을 보여 주기 위해 테스트 목적으로만 RDP를 사용하고 있습니다.
    1. **추가**를 선택합니다.

1. 규칙이 프로비전되면 인바운드 규칙 목록에 표시됩니다(화면을 새로 고침해야 할 수 있습니다). 새로 추가된 규칙에 경고 기호가 표시됩니다.  위에서 설명한 대로 NSG의 기능을 보여 주기 위해 테스트 목적으로만 RDP를 사용하고 있습니다. 새로 추가된 규칙을 선택합니다.
...
1. 페이지 맨 위에서 **연결**을 선택하고 **RDP**를 선택합니다.

1. IP 주소가 공용 IP 주소로 설정되어 있는지 확인합니다. 기본 포트 번호는 그대로 두고 **DRP 파일 다운로드**를 선택합니다.

1. 표시되는 팝업 창에서 **파일 열기**를 선택합니다.

1. 원격 데스크톱 연결 창이 열리면 **연결**을 선택합니다.

1. 그러면 자격 증명을 묻는 메시지가 표시됩니다.  사용자 이름 및 암호를 입력합니다(랩 지침 패널의 리소스 탭 참조).
...
1. 시작 화면에서 몇 초 후에 디바이스에 대한 개인 정보 설정을 선택하는 창이 나타납니다. **수락**을 선택합니다.  네트워크 창에서 **아니요**를 선택합니다.

1. VM이 가동되고 실행되면 VM에서 인터넷에 대한 아웃바운드 연결을 테스트합니다.
...
이전 작업에서 VM에 대한 RDP 연결을 설정할 수 있음을 확인했습니다. VM에서 인터넷에 아웃바운드 연결을 설정할 수도 있다는 것도 확인했습니다.  NSG에 대한 기본 아웃바운드 규칙이 아웃바운드 인터넷 트래픽을 허용하기 때문에 아웃바운드 인터넷 트래픽이 허용되었습니다.  이 작업에서는 나가는 인터넷 트래픽을 차단하고 해당 규칙을 테스트하는 사용자 지정 아웃바운드 규칙을 만드는 프로세스를 수행합니다.

1. SC900-WinVM | 네트워킹 페이지에 있어야 합니다. 이전에 브라우저 탭을 닫은 경우 페이지 맨 위에 있는 파란색 검색 창을 선택하고 가상 머신을 선택한 다음 VM, **SC900-WinVM**을 선택한 후 **네트워킹**을 선택합니다.
