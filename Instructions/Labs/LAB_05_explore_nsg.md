...
1. 페이지 위쪽에서 **연결**을 선택합니다.  IP 주소 아래의 **액세스 확인** 옵션을 선택합니다.  이 확인은 RDP 연결용 포트인 포트 3389를 사용하여 수행됩니다.  확인을 수행하면 "확인할 수 없음" 메시지가 표시됩니다.  네이티브 RDP 상자에서 **선택**을 클릭합니다.  열리는 창의 "1 네이티브 RDP의 필수 구성 요소 구성" 아래에 "VM에 연결하려면 Azure에서 몇 가지 기능을 구성해야 합니다."가 표시됩니다.  다음 작업에서 RDP 연결을 명시적으로 허용하도록 NSG를 설정합니다.
...
1. 열려 있는 NSG 탭에서 페이지 위쪽의 **홈** 링크를 *마우스 오른쪽 단추로 클릭*하고 **새 탭에서 링크 열기**를 선택하여 다른 Azure 서비스 페이지를 엽니다.
...
1. 브라우저에서 SC900-WinVM – Microsoft Azure 탭을 엽니다. 이전에 브라우저 탭을 닫은 경우 새 브라우저 탭을 열고 **https://portal.azure.com**을 입력한 다음 **가상 머신,**, VM, **SC900-WinVM**을 차례로 선택합니다.
