...
1. Azure 서비스 홈 페이지가 표시되어 있어야 합니다.  이전에 브라우저를 닫았다면 Microsoft Edge를 엽니다. 주소 표시줄에 **portal.azure.com**을 입력하고 관리자 자격 증명을 사용하여 로그인합니다. 이전에 로그인한 적이 있는 경우 MFA의 일환으로 보조 인증을 수행하라는 메시지가 표시될 수 있습니다.  이전에 로그인한 적이 없는 경우 MFA를 설정하라는 메시지가 표시될 수 있습니다.  화면의 프롬프트에 따라 MFA를 설정합니다.
