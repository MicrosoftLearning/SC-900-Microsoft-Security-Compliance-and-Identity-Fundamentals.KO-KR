...
6. 페이지 위쪽에서 **암호 재설정**을 선택합니다. 이전에 Diego로 로그인한 적이 없으므로 이 사용자의 암호를 알 수 없으므로 암호를 재설정해야 합니다.
...
11. 브라우저 페이지는 후속 작업에 필요하므로 계속 열어 둡니다.
...
4. 이제 Privileged Identity Management 빠른 시작 창이 표시됩니다.  **관리**를 선택합니다.

5. Contoso 역할 페이지가 표시됩니다.  페이지 위쪽의 검색 창에 **사용자**를 입력합니다.  검색 결과에서 **사용자 관리자**를 선택합니다.
...
11. 구성원 선택 아래에 선택한 구성원 1명과 선택한 구성원인 Diego Siciliani의 이름과 메일이 표시됩니다. 할당 추가 페이지 아래쪽에서 **다음**을 선택합니다.  

12. 이제 설정 페이지가 표시됩니다.  할당 유형은 기본 설정인 적격으로 유지합니다.
...
1. 왼쪽 탐색 패널에서 **내 역할**을 선택합니다.  이제 Azure AD 역할 정보가 표시됩니다.  Diego에게 사용자 관리자 역할이 할당되어 있음을 확인할 수 있습니다.  
...
1. 이제 Contoso Azure Active Directory 페이지로 돌아갑니다.  Azure AD 사용자 관리자 역할이 활성화되었으므로 사용자와 그룹 만들기, 라이선스 관리 등을 수행할 수 있습니다.   왼쪽 탐색 패널에서 **사용자**를 선택합니다.
